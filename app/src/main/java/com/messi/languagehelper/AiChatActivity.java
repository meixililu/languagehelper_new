...
public class AiChatActivity extends BaseActivity {
    ...
    private void initData() {
        ...
        beans.addAll(DataBaseUtil.getInstance().getAiEntityList());
    ...
    public void onSwipeRefreshLayoutRefresh() {
        List<AiEntity> list = DataBaseUtil.getInstance().getAiEntityList(beans.get(0).getId());
        if (list.size() > 0) {
            beans.addAll(0, list);
    ...
    private void requestData(String msg) {
        showProgressbar();
        String url = Settings.AiBrainUrl + "001" + "&msg=" + msg;
