...
public class ProverbFragment extends BaseFragment {
	...
	private void requestData() {
		isLoading = true;
		showProgressbar();
		LanguagehelperHttpClient.get(Setings.WHYYApi, new UICallback(getActivity()) {
			@Override
	...
	public void onViewClicked() {
		if (mTwistaItem != null) {
			if (!mTwistaItem.isShowResult()) {
				if(count == 0){
					answer.setText(mTwistaItem.getBehind() + "\n\n\n\n\n" + "轻触看下一条");
					count++;
