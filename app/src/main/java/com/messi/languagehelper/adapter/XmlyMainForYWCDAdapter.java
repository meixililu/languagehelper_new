...
public class XmlyMainForYWCDAdapter extends FragmentPagerAdapter {

    private FragmentProgressbarListener listener;
    private Context mContext;

    public XmlyMainForYWCDAdapter(FragmentManager fm, Context mContext, FragmentProgressbarListener listener) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        if(position == 0){
            ...
            return XmlySearchAlbumForYYSFragment.newInstance("语文");
        }else if(position == 2) {
    ...
    public CharSequence getPageTitle(int position) {
        if(position == 0){
            ...
            return mContext.getResources().getString(R.string.title_yuwen);
        }else if(position == 2) {
